...
const RelatedProfessional = ({ speciality, spId }) => {
  ...
  useEffect(() => {
    if (professionals.length > 0 && speciality) {
      const professionalsData = professionals.filter(
        (prof) => prof?.speciality === speciality && prof?._id !== spId
      );
      setRelProf(professionalsData);
    }
  }, [professionals, spId, speciality]);
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      <h1 className="text-3xl font-medium ">Top service Professionals to Book</h1>
...
export default RelatedProfessional;
