...
// import doctorModel from "../models/serviceProviderModel.js";
...
const bookAppointment = async (req, res) => {
  try {
    const { userId, spId, slotDate, slotTime } = req.body;

    const spData = await serviceProviderModel
      .findById(spId)
      .select("-password");
    ...
    let slots_booked = spData.slots_booked;
    ...
    await serviceProviderModel.findByIdAndUpdate(spId, { slots_booked });
...
const cancelAppointment = async (req, res) => {
  try {
    ...
    let slots_booked = sp.slots_booked;

    slots_booked[slotDate] = slots_booked[slotDate].filter(
      (e) => e !== slotTime
    );

    await doctorModel.findByIdAndUpdate(spId, { slots_booked });
